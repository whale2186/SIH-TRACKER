...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider mb-3 pb-2 border-b border-border flex items-center gap-2">
      {children}
    </h3>
  );
}
...
function InfoRow({ label, children, href }: { label: string; children: React.ReactNode; href?: string }) {
  return (
    <div className="flex flex-col sm:flex-row gap-2 sm:items-center py-2 border-b border-border/50 last:border-0">
      <span className="text-sm text-muted-foreground font-medium w-full sm:w-32 flex-shrink-0">{label}</span>
      {href ? (
        <a href={href} target="_blank" rel="noopener noreferrer" className="text-sm text-foreground hover:text-muted-foreground flex items-center gap-1">
          {children}
...
export function ProblemDetailClient({ problem }: Props) {
  ...
  return (
    <div className="max-w-5xl mx-auto p-3 sm:p-4 md:p-6 pb-20 lg:pb-6">
      {/* Back button */}
      <Link href="/" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-muted-foreground mb-4">
        <ArrowLeft size={16} />
        Back to tracker
      </Link>
      ...
      <div className="mb-4 sm:mb-6">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
          <div className="min-w-0">
            <span className="text-xs font-mono text-muted-foreground mb-1 block">{problem.psId}</span>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <Link
              ...
              className="p-2 sm:p-2.5 bg-muted text-muted-foreground hover:bg-gray-200 rounded-lg transition min-h-0"
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-xs sm:text-sm">
          <span className="px-2 py-0.5 bg-muted text-muted-foreground rounded capitalize">{problem.category}</span>
          <span className="px-2 py-0.5 bg-muted text-muted-foreground rounded">{problem.theme}</span>
          <span className="px-2 py-0.5 bg-muted text-muted-foreground rounded truncate max-w-[180px] sm:max-w-none">{problem.organization}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 sm:gap-3 mb-4 sm:mb-6">
        <div className="bg-card p-3 sm:p-4 rounded-lg border border-border">
          ...
          <p className="text-2xl sm:text-3xl font-bold text-foreground">{problem.applicationCount.toLocaleString()}</p>
          <p className="text-xs text-muted-foreground mt-1">of {problem.maxApplications.toLocaleString()} max</p>
        </div>
        <div className="bg-card p-3 sm:p-4 rounded-lg border border-border">
        ...
        <div className="bg-card p-3 sm:p-4 rounded-lg border border-border">
          ...
          <p className="text-xs text-muted-foreground mt-1">vs previous check</p>
        </div>
        <div className="bg-card p-3 sm:p-4 rounded-lg border border-border">
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <section>
          ...
          <div className="bg-card p-3 sm:p-4 rounded-lg border border-border prose prose-sm max-w-none">
            {problem.descriptionHtml
              ? <div className="ps-description text-sm text-muted-foreground whitespace-pre-wrap leading-relaxed" dangerouslySetInnerHTML={{ __html: problem.descriptionHtml }} />
          ...
          <section>
            ...
            <div className="bg-card p-3 sm:p-4 rounded-lg border border-border">
              <a
                ...
                className="text-sm text-foreground hover:text-muted-foreground underline flex items-center gap-1 break-all"
              >
                <ExternalLink size={12} />
                {problem.datasetUrl}
              </a>
        ...
        {problem.resourceLinks && (
          <section>
            ...
            <div className="bg-card p-3 sm:p-4 rounded-lg border border-border">
              <div className="prose prose-sm max-w-none text-muted-foreground" dangerouslySetInnerHTML={{ __html: problem.resourceLinks }} />
            </div>
          </section>
        )}
        ...
        {showChart && (
          <section>
            ...
            <div className="bg-card p-3 sm:p-4 rounded-lg border border-border h-48 sm:h-64">
          ...
          <section>
            ...
            <div className="sm:hidden space-y-3">
              {problem.history.slice(0, 20).map((h) => (
                <div key={h.id} className="bg-card rounded-lg border border-border p-4">
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <span className="text-xs font-mono text-muted-foreground">{formatDistanceToNow(new Date(h.timestamp), { addSuffix: true })}</span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div className="p-2 bg-muted/50 rounded">
                      <p className="text-[10px] text-muted-foreground/70 uppercase tracking-wider">Count</p>
                    ...
                    <div className="p-2 bg-muted/50 rounded">
                      ...
                      <span className={clsx("text-sm font-medium", h.change > 0 ? "text-green-600" : h.change < 0 ? "text-red-600" : "text-muted-foreground/70")}>
                        {h.change > 0 ? "+" : ""}{h.change}
                      </span>
                    </div>
                    <div className="p-2 bg-muted/50 rounded">
                      <p className="text-[10px] text-muted-foreground/70 uppercase tracking-wider">Source</p>
                      <p className="text-sm text-muted-foreground capitalize">{h.source}</p>
                    </div>
                  </div>
                </div>
              ))}
                ...
                <tbody>
                  {problem.history.slice(0, 20).map((h) => (
                    <tr key={h.id} className="border-b border-border/50 hover:bg-muted/50">
                      ...
                      <td className="px-3 py-2 text-sm font-mono font-medium text-foreground">
        ...
        <section>
          ...
          <div className="bg-card p-3 sm:p-4 rounded-lg border border-border">
            {problem.shortlist ? (
              ...
              <button
                onClick={handleShortlistToggle}
                className="w-full py-3 text-sm text-muted-foreground hover:text-foreground border-2 border-dashed border-input rounded-lg hover:border-gray-400"
              >
        ...
        <section>
          ...
          <div className="bg-card p-3 sm:p-4 rounded-lg border border-border space-y-2 text-sm">
